...
{
    //
    public function adtrpall(Request $request)
    {
        ...
        $logs = PlayoutLog::where('date', $date)
                ->get();
        foreach($logs as $log){
            //return response()->json(["value" => $log->channel], 200);
            $start=$log->start;
            $finish=$log->finish;
            ...
            unset($viewer);
            $viewer = array();
            $adtrparr=["commercial_name"=>$log->commercial_name,"program"=>$log->program,"channel_id"=>$log->channel_id,"date"=>$log->date,
            "start"=>$log->start,"finish"=>$log->finish,"timewatched"=>$timewatched,"duration"=>$log->duration
            ,"tvrp"=>$tvrp,"tvr0"=>$tvr0
            ,"reach0"=>$reach0,"reachp"=>$reachp,"playlog_id"=>$log->id];
